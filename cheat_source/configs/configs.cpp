...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.ragebot.lowdelta_resolver, key_bind(TOGGLE), crypt_str("Ragebot.lowdelta_resolver"));
	...
	setup_item(&g_cfg.esp.scope_style, 8, crypt_str("Esp.world_crosshair"));
	...
	setup_item(&g_cfg.menu.watercolor1, Color(117, 122, 202), crypt_str("Menu.watercolor1"));
	setup_item(&g_cfg.menu.watercolor2, Color(117, 122, 202), crypt_str("Menu.watercolor2"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\Wasovski\\Configs\\") + config;
		}
...
void C_ConfigManager::config_files()
{
	...
	std::string path = folder + crypt_str("/*.wz");
	WIN32_FIND_DATA fd;
