...
void otheresp::indicators()
{
	...
	if (g_cfg.esp.indicators[INDICATOR_FAKE] && (antiaim::get().type == ANTIAIM_LEGIT || g_cfg.antiaim.type[antiaim::get().type].desync))
	{
		...
		m_indicators.push_back(m_indicator("SYNC", color));
	}
	...
	{
		m_indicators.push_back(m_indicator(("FL: " + std::to_string(fakelag::get().max_choke)), Color(130, 170, 20)));
	...
	if (g_cfg.esp.indicators[INDICATOR_DT] && g_cfg.ragebot.double_tap && g_cfg.ragebot.double_tap_key.key > KEY_NONE && g_cfg.ragebot.double_tap_key.key < KEY_MAX && misc::get().double_tap_key)
		m_indicators.push_back(m_indicator("RAPID FIRE", !g_ctx.local()->m_bGunGameImmunity() && !(g_ctx.local()->m_fFlags() & FL_FROZEN) && !antiaim::get().freeze_check && misc::get().double_tap_enabled && !weapon->is_grenade() && weapon->m_iItemDefinitionIndex() != WEAPON_TASER && weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER && weapon->can_fire(false) ? Color(130, 170, 20) : Color(130, 20, 20)));
	...
	if (g_cfg.esp.indicators[INDICATOR_LDR] && key_binds::get().get_key_bind_state(23))
		m_indicators.push_back(m_indicator("LOWDELTA RESOLVER", Color(130, 170, 20)));
}
...
void otheresp::automatic_peek_indicator()
{
	...
	if (key_binds::get().get_key_bind_state(18) || alpha)
	{
		...
		render::get().Draw3DFilledCircle(position, 20.0f, g_ctx.globals.fired_shot ? Color(30, 30, 30, (int)(alpha * 255.0f)) : Color(10, 10, 10, (int)(alpha * 255.0f)));
