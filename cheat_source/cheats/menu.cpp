...
std::string get_config_dir()
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, 0x001a, NULL, NULL, path)))
		folder = std::string(path) + crypt_str("\\Wasovski\\Configs\\");
...
void add_config()
{
	...
	if (g_cfg.new_config_name.find(crypt_str(".wz")) == std::string::npos)
		g_cfg.new_config_name += crypt_str(".wz");
...
bool LabelClick(const char* label, bool* v, const char* unique_id)
{
	...
	if (*v)
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(RGBA_TO_FLOAT(117, 122, 202, 255)));
...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\Wasovski\\Scripts\\");
	...
	if (!rage_section)
	{
		if (!child)
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("Pitch Resolver"), &g_cfg.ragebot.pitch_antiaim_correction);
				...
				draw_keybind(crypt_str("Lowdelta resolver"), &g_cfg.ragebot.lowdelta_resolver, crypt_str("##HOTKEY_LDR"));
		...
		if (!child)
		{
			if (!visuals_section)
			{
				...
				ImGui::BeginChild(crypt_str("##VISUAL1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
				{
					...
					{
						ImGui::Text("");
						ImGui::Text(u8"Если ты сука врубишь два скопа,");
						ImGui::Text(u8"я тебе винду снесу");
						ImGui::Text("");
						draw_multicombo(crypt_str("Scope style"), g_cfg.esp.scope_style, scope_style, ARRAYSIZE(scope_style),preview);
						ImGui::Text(crypt_str("Scope color "));
						ImGui::SameLine();
						ImGui::ColorEdit(crypt_str("##scopecolor"), &g_cfg.esp.scopecolor, ALPHA);
					}
	...
	if (!child)
	{
		if (!misc_section)
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				{
					padding(0, -5);
					ImGui::SliderFloat(crypt_str("Amount"), &g_cfg.misc.aspect_ratio_amount, 0.5f, 2.0f);
				}
			}
			ImGui::EndChild();
			tab_end();
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				{
					ImGui::Text(crypt_str("Gradient Color #1 "));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##watercolor1"), &g_cfg.menu.watercolor1, ALPHA);

					ImGui::Text(crypt_str("Gradient Color #2 "));
					ImGui::SameLine();
					ImGui::ColorEdit(crypt_str("##watercolor2"), &g_cfg.menu.watercolor2, ALPHA);
				}
		...
		ImGui::BeginChild(crypt_str("##LUA_FIRST"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
		{
			...
			if (ImGui::CustomButton(crypt_str("Open scripts folder"), crypt_str("##LUAS__FOLDER"), ImVec2(291 * dpi_scale, 26 * dpi_scale)))
			{
				...
				auto get_dir = [&folder]() -> void
				{
					...
					if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
						folder = std::string(path) + crypt_str("\\Wasovski\\Scripts\\");
...
void c_menu::draw(bool is_open)
{
	...
	// start menu render
	ImGui::Begin(crypt_str("Wasovski"), nullptr, ImGuiWindowFlags_NoDecoration);
	{
		...
		{																			 // KEYWORD : TABS_FIND
			...
			if (ImGui::MainTab(crypt_str("Rage"), super_ico, crypt_str("A"), active_tab == 0, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 0;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Legit"), super_ico, crypt_str("B"), active_tab == 1, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 1;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Visuals"), super_ico, crypt_str("C"), active_tab == 2, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 2;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Players"), super_ico, crypt_str("D"), active_tab == 3, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 3;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Misc"), super_ico, crypt_str("E"), active_tab == 4, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 4;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Settings"), super_ico, crypt_str("F"), active_tab == 5, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 5;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Player List"), super_ico, crypt_str("G"), active_tab == 6, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 6;
			ImGui::Spacing();
			if (ImGui::MainTab(crypt_str("Lua"), super_ico, crypt_str("E"), active_tab == 7, ImVec2(124 * dpi_scale, 0)) && last_tab == active_tab && !preview_reverse) active_tab = 7;
		...
		ImGui::SetCursorPos(ImGui::GetCursorPos() + ImVec2(850 * dpi_scale - ImGui::CalcTextSize(crypt_str("Wasovski")).x - (11 * dpi_scale), (16 * dpi_scale) - (ImGui::CalcTextSize(crypt_str("Wasovski")).y / 2)));

		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, m_alpha);
		ImGui::Text(crypt_str("Wasovski"));
		ImGui::PopStyleVar();
		ImGui::PopStyleColor();
