...
#if RELEASE
#if BETA
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ Zerus BETA ] ")); //-V807
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ Zerus ] "));
#endif
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("Wasovski | ")); //-V807
#endif
...
#if RELEASE
#if BETA
		auto log = crypt_str("[ \x0CZerus BETA \x01] ") + text;
		chat->chat_print(log.c_str());
#else
		auto log = crypt_str("[ \x0CZerus \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
#else
		auto log = crypt_str("\x0CWasovski | \x01 ") + text;
		chat->chat_print(log.c_str());
#endif
	}
}