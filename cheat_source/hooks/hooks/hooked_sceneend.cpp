...
void __fastcall hooks::hooked_sceneend(void* ecx, void* edx)
{
	...
	g_ctx.local((player_t*)m_entitylist()->GetClientEntity(m_engine()->GetLocalPlayer()), true); //-V807Wasovski_
	...
	for (auto i = 1; i <= m_entitylist()->GetHighestEntityIndex(); i++)
	{
		...
		{
			m_materialsystem()->FindMaterial(crypt_str("Wasovski_chams"), nullptr), //-V807
			m_materialsystem()->FindMaterial(crypt_str("Wasovski_metallic"), nullptr),
			m_materialsystem()->FindMaterial(crypt_str("Wasovski_flat"), nullptr),
			...
			m_materialsystem()->FindMaterial(crypt_str("chamsGlow"), nullptr),
			m_materialsystem()->FindMaterial(crypt_str("models/inventory_items/wildfire_gold/wildfire_gold_detail"), nullptr),
			m_materialsystem()->FindMaterial(crypt_str("Wasovski_glow"), nullptr)
		};
