...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (reload_fonts())
	{
		...
		fonts[LOGS] = create_font(crypt_str("Lucida Console"), 10, FW_DONTCARE, FONTFLAG_DROPSHADOW);
		fonts[ESP] = create_font(crypt_str("mainesp"), 11, FW_DONTCARE, FONTFLAG_ANTIALIAS);
		fonts[NAME] = create_font(crypt_str("mainesp"), 12, FW_DONTCARE, FONTFLAG_ANTIALIAS);
		...
		fonts[GRENADES] = create_font(crypt_str("undefeated"), 20, FW_DONTCARE, FONTFLAG_ANTIALIAS);
		fonts[INDICATORFONT] = create_font(crypt_str("mainesp"), 25, FW_DONTCARE, FONTFLAG_ANTIALIAS);
		fonts[DAMAGE_MARKER] = create_font(crypt_str("mainesp"), 15, FW_DONTCARE, FONTFLAG_ANTIALIAS);
	...
	if (panel_id == panel)
	{
		if (g_ctx.available())
		{
			...
			{
				if (g_cfg.player.enable && g_cfg.esp.scope_style[OFF_CROSS] && g_ctx.globals.scoped && weapon->is_sniper())
				{

				};

				if (g_cfg.player.enable && g_cfg.esp.scope_style[NONE_CROSS] && g_ctx.globals.scoped && weapon->is_sniper())
				{
					static int w, h;
				...
				if (g_cfg.player.enable && g_cfg.esp.scope_style[STYLE_1] && g_ctx.globals.scoped && weapon->is_sniper())
				{
					static int w, h;
					m_engine()->GetScreenSize(w, h);
					auto color = Color(g_cfg.esp.scopecolor.r(), g_cfg.esp.scopecolor.g(), g_cfg.esp.scopecolor.b(), 240);
					auto end_color = Color(0, 0, 0, 0);
				...
				if (g_cfg.player.enable && g_cfg.esp.scope_style[STYLE_2] && g_ctx.globals.scoped && weapon->is_sniper())
				{
					static int w, h;
					m_engine()->GetScreenSize(w, h);
					auto color = Color(g_cfg.esp.scopecolor.r(), g_cfg.esp.scopecolor.g(), g_cfg.esp.scopecolor.b(), g_cfg.esp.scopecolor.a());
					auto end_color = Color(0, 0, 0, 0);
					render::get().gradient(w / 2, h / 2 - 165, 1, 150, color, end_color, GRADIENT_VERTICAL); //up
					render::get().gradient(w / 2 - 120, h / 2, 150, 1, color, end_color, GRADIENT_HORIZONTAL); //left
					render::get().gradient(w / 2 + 19, h / 2, 150, 1, end_color, color, GRADIENT_HORIZONTAL); //right
					render::get().gradient(w / 2, h / 2 + 15, 1, 150, end_color, color, GRADIENT_VERTICAL); //down
				}
			}
